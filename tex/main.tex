...
\title{Q-DARE: Quantization-Aligned Caching with\\
Dimension-Adaptive Reconfigurable Execution for\\
...
\begin{abstract}
Video Diffusion Transformers (VDiTs) incur high inference cost from iterative denoising. Caching and low-bit quantization reduce the number and cost of recomputed steps, respectively, but their direct composition exposes two coupled inefficiencies: full-precision cache indicators can be misaligned with quantized execution, while asymmetric spatial/temporal attention shapes fragment fixed-width processing arrays. We present \textbf{Q-DARE}, an algorithm--hardware co-design combining \emph{quantization-aligned caching} (QAC) with a \emph{dimension-adaptive reconfigurable engine} (DARE). QAC evaluates and calibrates cache relevance in the quantized domain, improving indicator correlation from \FPIndicatorCorr{} to \QIndicatorCorr{} and cache reuse from \BaseCacheRatio{} to \QACCacheRatio{} at matched generation quality. DARE adapts output parallelism for $QK^{\mathsf T}$ and reduction parallelism for $PV$ on a shared datapath, mitigating shape fragmentation without duplicating the arithmetic array. On \ModelName{}, Q-DARE achieves \QACVBench{} VBench versus \BaselineVBench{} for the 16-bit baseline. RTL implementation in \TechNode{} delivers \EtoESpeedup{} end-to-end speedup and \EnergyGain{} higher energy efficiency with \AreaOverhead{} area overhead.
\end{abstract}
